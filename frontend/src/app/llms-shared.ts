...
export async function buildLlmsText({ full }: { full: boolean }) {
  ...
  return [
    `# ${app}`,
    '',
    `${app} okul oncesi Ingilizce, hikaye temelli egitim setleri, atolye programlari, ev ve ozel ders cozumleri, Woody Academy ve dijital icerik alanlari sunan cocuk odakli egitim markasidir.`,
    '',
    '## Site yapisi',
    ...
    'Sitede Organization, WebSite, EducationalOrganization, Article, FAQPage, LocalBusiness ve BreadcrumbList JSON-LD semalari kullanilir. Magaza teklif bazlidir; fiyat verisi olmadan Product + Offer semasi uretilmez.',
    '',
    '## Urun ve hizmetler',
    '- Okul oncesi Ingilizce egitim setleri',
    '- Atolye ve etkinlik programlari',
