...
export async function loadDbBlogPosts(locale: string): Promise<WoodyDbBlogPost[]> {
  try {
    const res = await fetch(`${apiBase()}/blog?locale=${encodeURIComponent(locale)}&limit=12`, {
