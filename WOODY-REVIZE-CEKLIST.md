...
- [ ] <html lang> refactor — SAHIP: bu oturum (peer devretti, 2026-08-30 akşam). PLAN: route group'lu çift root layout — `app/(locale)/[locale]/layout.tsx` kendi `<html lang={locale}>`'i ile root olur (lang `params.locale`'den, ASLA headers() — [[woody-static-headers-500-trap]]); `app/(root)/layout.tsx` minimal `<html lang="tr">` ile kök page.tsx/not-found/global-error'ı taşır; `app/layout.tsx` silinir (Next çoklu root layout route group'la). Refactor sonrası ae159de'nin x-pathname okuması gereksizleşirse kaldırılır. Aciliyet düşük: hreflang doğru, client effect UX'i düzeltiyor; kayıp yalnız crawler'ın ilk HTML lang sinyali
