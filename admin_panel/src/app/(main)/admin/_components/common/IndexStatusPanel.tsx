...
import { useGscEntityIndexQuery, useGscEntityInspectMutation, useGscStatusQuery } from "@/integrations/hooks";
import { GSC_CATEGORY_META, GSC_VERDICT_LABELS, type GscIndexItem } from "@/integrations/shared";
...
export function IndexStatusPanel({
  ...
  return (
    <div className="overflow-hidden rounded-[28px] border border-gm-border-soft bg-gm-surface/20">
      <div className="flex items-center justify-between gap-3 border-gm-border-soft border-b bg-gm-surface/40 px-6 py-4">
        <div className="flex items-center gap-2">
          <Globe className="size-4 text-gm-gold" />
          <span className="font-serif text-gm-text text-lg">Google İndeks Durumu</span>
        </div>
        <span
          className={cn(
            "inline-flex items-center gap-1 rounded-full border px-3 py-1 font-bold text-[11px]",
            meta.tone,
          )}
        >
          {meta.label}
      ...
      <div className="space-y-4 p-6">
        {!cleanSlug ? (
          <p className="font-serif text-gm-muted text-sm italic">
          ...
          <>
            <dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
              <div>
                <dt className="font-bold text-[10px] text-gm-muted uppercase tracking-widest">Sonuç</dt>
                <dd className="mt-1 text-gm-text">
                  {item ? GSC_VERDICT_LABELS[item.verdict] || item.verdict || "—" : "Denetlenmedi"}
                </dd>
              </div>
              <div>
                <dt className="font-bold text-[10px] text-gm-muted uppercase tracking-widest">Kapsam durumu</dt>
                <dd className="mt-1 text-gm-text">{item?.coverage_state || "—"}</dd>
              </div>
              <div>
                <dt className="font-bold text-[10px] text-gm-muted uppercase tracking-widest">Son tarama (Google)</dt>
                <dd className="mt-1 font-mono text-gm-text text-xs">{formatDate(item?.last_crawl)}</dd>
              </div>
              <div>
                <dt className="font-bold text-[10px] text-gm-muted uppercase tracking-widest">Son denetim</dt>
                <dd className="mt-1 font-mono text-gm-text text-xs">{formatDate(item?.checked_at)}</dd>
              </div>
            </dl>

            <div className="rounded-2xl border border-gm-border-soft bg-gm-bg-deep/30 p-4">
              <div className="mb-1 font-bold text-[10px] text-gm-muted uppercase tracking-widest">Neden / Öneri</div>
              <p className="text-gm-text/90 text-sm leading-relaxed">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Button
                ...
                className="h-11 rounded-full px-6 font-bold text-[10px] uppercase tracking-widest"
              >
              ...
              {gscHref ? (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 font-bold text-[11px] text-gm-muted uppercase tracking-widest transition-colors hover:text-gm-gold"
                >
              ...
              {category === "issue" ? (
                <span className="inline-flex items-center gap-1.5 font-bold text-[11px] text-gm-error">
