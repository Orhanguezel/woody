...
function collectMediaFields(settingKey: string, value: unknown): MediaFieldSpec[] {
  const normalizedKey = String(settingKey || "")
    .trim()
    .toLowerCase();
  ...
  return out.filter(
    (item, index, arr) => arr.findIndex((candidate) => candidate.path.join(".") === item.path.join(".")) === index,
  );
}
...
const JsonStructuredRenderer: React.FC<StructuredRenderProps> = ({ value, setValue, disabled }) => {
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-2xl border border-gm-border-soft p-3 text-sm text-gm-muted">
...
const CompanyStructuredRenderer: React.FC<StructuredRenderProps> = ({ value, setValue, disabled }) => {
  ...
  const seed = React.useMemo(() => ({ company_name: "", slogan: "", about: "" }) as any, []);
...
export default function SiteSettingsDetailClient({ id }: { id: string }) {
  ...
  const rowFromList: SiteSetting | null = React.useMemo(() => {
    ...
    const exact = arr.find((r) => String(r?.key || "") === settingKey && String(r?.locale || "") === selectedLocale);
        ...
        <Card className="bg-gm-surface/20 border-gm-border-soft rounded-[32px] overflow-hidden backdrop-blur-sm shadow-xl">
          <CardHeader className="p-10 text-center">
            <CardTitle className="font-serif text-3xl text-gm-text">
              {t("admin.siteSettings.detail.keyMissingTitle", null, "Ayar Anahtarı Eksik")}
            </CardTitle>
            <CardDescription className="text-gm-muted italic font-serif opacity-70 text-lg mt-2">
              {t("admin.siteSettings.detail.keyMissingDesc", null, "Lütfen geçerli bir ayar anahtarı seçin.")}
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <Card className="bg-gm-surface/20 border-gm-border-soft rounded-[32px] overflow-hidden backdrop-blur-sm shadow-xl">
          <CardHeader className="p-10 text-center">
            <CardTitle className="font-serif text-3xl text-gm-text">
              {t("admin.siteSettings.detail.localesMissingTitle", null, "Dil Seçenekleri Bulunamadı")}
            </CardTitle>
            <CardDescription className="text-gm-muted italic font-serif opacity-70 text-lg mt-2">
          ...
          <CardContent className="flex justify-center pb-10">
            <Button
              asChild
              variant="outline"
              className="rounded-full border-gm-border-soft hover:bg-gm-surface/40 h-12 px-8 text-[10px] font-bold tracking-widest uppercase"
            >
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
            ...
            >
              <Link prefetch={false} href={backHref}>
                ...
                <svg
                  ...
                  className="lucide lucide-arrow-left size-4 group-hover:-translate-x-1 transition-transform"
                >
          ...
          <h1 className="font-serif text-4xl text-gm-text flex items-center gap-3">
            ...
            <code className="text-gm-gold bg-gm-gold/10 px-3 py-1 rounded-xl text-2xl border border-gm-gold/20 shadow-sm">
              {settingKey}
            </code>
          </h1>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div className="space-y-2">
            <Label className="text-[10px] font-bold text-gm-muted tracking-[0.2em] uppercase ml-1 block">
              {t("admin.siteSettings.detail.localeLabel", null, "Dil Seçimi")}
            </Label>
            <Select
              ...
              <SelectContent className="bg-gm-bg-deep border-gm-border-soft rounded-2xl shadow-2xl">
                {localeOptions.map((o) => (
                  <SelectItem
                    key={o.value}
                    value={o.value}
                    className="rounded-xl focus:bg-gm-gold/10 focus:text-gm-gold"
                  >
                    {o.label}
                  </SelectItem>
                ))}
          ...
          {selectedLocale && (
            <Badge
              variant="outline"
              className="border-gm-gold/30 bg-gm-gold/5 text-gm-gold px-4 py-2 text-[10px] font-bold uppercase tracking-[0.2em] h-12"
            >
              {selectedLocale}
            </Badge>
          )}
          {busy && (
            <Badge
              variant="outline"
              className="border-gm-border-soft bg-gm-surface/40 text-gm-muted px-4 py-2 text-[10px] font-bold uppercase tracking-[0.2em] h-12"
            >
        ...
        <div className="space-y-6">
          {isFallback ? (
            <div className="rounded-[24px] border border-gm-gold/30 bg-gm-gold/5 p-6 text-sm text-gm-gold font-serif italic backdrop-blur-sm shadow-inner text-center">
              {t(
                "admin.siteSettings.detail.fallbackNotice",
                {
                  selectedLocale,
                  effectiveLocale,
                },
          ...
          {!row && !busy ? (
            <div className="rounded-[24px] border border-gm-border-soft bg-gm-surface/20 p-6 text-sm text-gm-muted font-serif italic backdrop-blur-sm text-center">
              {t(
                "admin.siteSettings.detail.noRecordNotice",
                { key: settingKey, locale: selectedLocale },
                `Kayıt bulunamadı.`,
              )}
            </div>
