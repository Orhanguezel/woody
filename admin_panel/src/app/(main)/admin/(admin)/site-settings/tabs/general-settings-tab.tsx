...
export const GeneralSettingsTab: React.FC<GeneralSettingsTabProps> = ({ locale, settingPrefix }) => {
  ...
  const createMissing = async () => {
    try {
      ...
      toast.success(
        t("admin.siteSettings.general.globalBootstrapSuccess", {}, "Eksik anahtarlar boş varsayılanlarla oluşturuldu."),
      );
      await refetchAll();
    } catch (err: any) {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-3">
                  ...
                  {!r.hasValue ? (
                    <Badge
                      variant="outline"
                      className="text-[9px] text-gm-muted uppercase tracking-[0.2em] border-gm-border-soft bg-gm-bg-deep"
                    >
      ...
      {hasAnyMissing ? (
        ...
        >
          ...
          {t("admin.siteSettings.general.globalBootstrap", null, "Eksik anahtarları boş varsayılanla oluştur")}
        </Button>
      ) : null}
    </div>
  );
