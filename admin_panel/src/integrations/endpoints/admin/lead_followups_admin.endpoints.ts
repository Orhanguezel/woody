...
const api = baseApi.injectEndpoints({endpoints:(build)=>({
  listLeadFollowups:build.query<{items:(LeadFollowup & {record_kind:'quote'|'contact';record_id:string})[];counts:{stage:string;count:number}[];scope:string},void>({query:()=>'/admin/lead-followups',providesTags:[{type:'Contacts',id:'followups:LIST'}]}),
