...
export function normalizeAdminUiCopy(raw: unknown): AdminUiCopy {
  ...
  const items: AdminNavCopy['items'] = {
    ...
    paytr_logs: uiText(itemsRaw.paytr_logs),
    quote_requests: uiText(itemsRaw.quote_requests),
    products: uiText(itemsRaw.products),
    series: uiText(itemsRaw.series),
    levels: uiText(itemsRaw.levels),
    entitlements: uiText(itemsRaw.entitlements),
